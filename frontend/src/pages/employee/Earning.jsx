...
const EmployeeSalaryView = () => {
  ...
  useEffect(() => {
    const fetchSalaries = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${API_BASE_URL}/salary/my-salaries`, { withCredentials: true });
        if (response.data.success) {
          setSalaries(response.data.data);
          setError(null);
          if (response.data.data.length > 0) fetchSalaryDetails(response.data.data[0]._id);
        } else {
          throw new Error(response.data.message || 'Failed to load salary data');
        }
      } catch (err) {
  ...
  const fetchSalaryDetails = async (id) => {
    try {
      ...
      console.error('Error fetching salary details:', err);
      setError(err.message || 'Failed to load salary details.');
    }
  };
  ...
  const generatePDF = async () => {
    ...
    try {
      setGeneratingPDF(true);
      const canvas = await html2canvas(salaryElement, { scale: 2, useCORS: true, logging: false });
      const pdf = new jsPDF('p', 'mm', 'a4');
      const imgWidth = 190;
      const pageHeight = 280;
      const imgHeight = (canvas.height * imgWidth) / canvas.width;
  ...
  const totalEarned = useMemo(() => salaries.reduce((acc, curr) => acc + (curr.calculations?.netSalary || 0), 0), [salaries]);

  if (loading) {
    return <div className="flex justify-center items-center h-screen bg-gray-50"><div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-blue-600"></div></div>;
  }

  return (
    // STRATEGY: REMOVE max-w restriction. Use w-full.
    <div className="w-full min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8">
      ...
      <div className="w-full mb-8">
        ...
        {/* Stats Grid - Forces 3 columns */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
          <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 w-full">
            <p className="text-xs font-semibold text-gray-500 uppercase">Total Earnings (YTD)</p>
            <h3 className="text-2xl font-bold text-gray-800 mt-1">{formatCurrency(totalEarned)}</h3>
          </div>
          <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 w-full">
            <p className="text-xs font-semibold text-gray-500 uppercase">Latest Payment</p>
            <h3 className="text-2xl font-bold text-blue-600 mt-1">
              {salaries.length > 0 ? formatDate(salaries[0].paymentDate || new Date()) : '-'}
            </h3>
          </div>
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-5 rounded-xl shadow-sm text-white w-full">
            <p className="text-xs font-medium text-blue-100 uppercase">Next Pay Date</p>
            <h3 className="text-2xl font-bold mt-1">Feb 01, 2026</h3>
          </div>
        </div>
      </div>

      {error && <div className="bg-red-50 text-red-700 p-4 mb-6 rounded-lg shadow-sm border-l-4 border-red-500">{error}</div>}

      {/* 2. MAIN CONTENT - FLEXBOX STRATEGY (Not Grid) */}
      <div className="flex flex-col lg:flex-row gap-6 w-full items-start">

        {/* LEFT PANEL: HISTORY LIST */}
        {/* Fixed width on desktop (w-96), full width on mobile */}
        <div className="w-full lg:w-96 flex-shrink-0 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sticky top-6">
              ...
              <ul className="divide-y divide-gray-100">
                {salaries.map((salary) => (
                  <li
                    key={salary._id}
                    onClick={() => fetchSalaryDetails(salary._id)}
                    className={`p-4 cursor-pointer transition-all hover:bg-blue-50 ${activeSalary && activeSalary._id === salary._id ? 'bg-blue-50 border-l-4 border-blue-600' : 'border-l-4 border-transparent'
                      }`}
                  >
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-sm font-semibold text-gray-800">{salary.period.label}</span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full uppercase font-medium ${salary.status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                        }`}>{salary.status}</span>
                    </div>
                    <p className="text-base font-bold text-gray-800">{formatCurrency(salary.calculations.netSalary)}</p>
                  </li>
                ))}
              </ul>
            )}
        ...
        {/* RIGHT PANEL: DETAILS SLIP */}
        {/* 'flex-1' commands this div to take ALL remaining space */}
          ...
          {activeSalary ? (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden w-full">
              ...
              <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
                <div>
                  <h2 className="text-xl font-bold text-gray-800">{activeSalary.period.label} Slip</h2>
                  <p className="text-gray-400 text-xs">ID: #{activeSalary._id.slice(-6).toUpperCase()}</p>
                </div>
                <button
                  onClick={generatePDF}
                  disabled={generatingPDF}
                  className="flex items-center px-4 py-2 bg-gray-900 text-white rounded-lg text-xs font-medium hover:bg-gray-800"
                >
                  {generatingPDF ? '...' : <><DownloadIcon /> PDF</>}
                </button>
              </div>

              {/* SLIP CONTENT */}
              <div id="salary-details" className="p-6 lg:p-10 w-full">

                {/* Slip Header */}
                <div className="flex flex-col md:flex-row justify-between md:items-end mb-8 pb-6 border-b border-gray-100 gap-4">
                  <div>
                    <h1 className="text-2xl font-bold text-blue-600">AutoFiks</h1>
                    <p className="text-gray-400 text-xs mt-1">Employee Salary Record</p>
                  </div>
                  <div className="text-left md:text-right">
                    <p className="text-xs text-gray-500 uppercase tracking-wide">Net Pay</p>
                    <p className="text-3xl font-bold text-gray-900">{formatCurrency(activeSalary.calculations.netSalary)}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="text-sm font-bold text-gray-800 mb-3">Attendance</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between text-gray-600"><span>Working Days</span> <span className="font-medium text-gray-900">{activeSalary.workingDays.total}</span></div>
                      <div className="flex justify-between text-gray-600"><span>Present</span> <span className="font-medium text-green-600">{activeSalary.workingDays.present}</span></div>
                      <div className="flex justify-between text-gray-600"><span>Absent</span> <span className="font-medium text-red-500">{activeSalary.workingDays.absent}</span></div>
                    </div>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="text-sm font-bold text-gray-800 mb-3">Leaves</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between text-gray-600"><span>Annual</span> <span className="font-medium text-gray-900">{activeSalary.workingDays.leave.annual}</span></div>
                      <div className="flex justify-between text-gray-600"><span>Sick</span> <span className="font-medium text-gray-900">{activeSalary.workingDays.leave.sick}</span></div>
                      <div className="flex justify-between text-gray-600"><span>Casual</span> <span className="font-medium text-gray-900">{activeSalary.workingDays.leave.casual}</span></div>
                    </div>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="text-sm font-bold text-gray-800 mb-3">Status</h4>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between text-gray-600"><span>Status</span> <span className="capitalize font-medium text-gray-900">{activeSalary.status}</span></div>
                      <div className="flex justify-between text-gray-600"><span>From</span> <span className="font-medium text-gray-900">{formatDate(activeSalary.period.startDate)}</span></div>
                      <div className="flex justify-between text-gray-600"><span>To</span> <span className="font-medium text-gray-900">{formatDate(activeSalary.period.endDate)}</span></div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <h3 className="text-sm font-bold text-gray-800 border-b pb-2 mb-3">Earnings</h3>
                    <div className="bg-green-50 p-3 rounded border border-green-100 flex justify-between items-center">
                      ...
                      <span className="text-sm font-bold text-gray-900">{formatCurrency(activeSalary.calculations.basicPayment)}</span>
                    </div>
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-gray-800 border-b pb-2 mb-3">Deductions</h3>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between text-gray-500"><span>Absence / Late</span> <span className="text-red-600 font-medium">-{formatCurrency(activeSalary.calculations.deductions.absences)}</span></div>
                      <div className="flex justify-between text-gray-500"><span>Tax</span> <span className="text-red-600 font-medium">-{formatCurrency(activeSalary.calculations.deductions.tax)}</span></div>
                      {activeSalary.calculations.deductions.other > 0 && (
                        <div className="flex justify-between text-gray-500"><span>Other</span> <span className="text-red-600 font-medium">-{formatCurrency(activeSalary.calculations.deductions.other)}</span></div>
                      )}
                      <div className="mt-2 bg-red-50 p-3 rounded border border-red-100 flex justify-between items-center font-bold text-red-700">
                        <span>Total Deductions</span>
                        <span>-{formatCurrency(activeSalary.calculations.deductions.absences + activeSalary.calculations.deductions.tax + activeSalary.calculations.deductions.other)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 text-right">
                  <p className="text-xs text-gray-400 uppercase">Total Net Payable</p>
                  <h2 className="text-3xl font-bold text-blue-600">{formatCurrency(activeSalary.calculations.netSalary)}</h2>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center h-full flex flex-col items-center justify-center">
              ...
              <h3 className="text-lg font-medium text-gray-900">Select a Salary Record</h3>
            </div>
          )}
        </div>
      </div>
