...
const ServiceTicket = () => {
  ...
  // Fetch all appointments from the API when the component mounts
  useEffect(() => {
    fetchAppointments();
  ...
  const fetchAppointments = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://localhost:8070/api/bookings/all');  // Fetch all appointments (admin only)
  ...
  const updateStatus = async (id, status) => {
    try {
      // Call the new API endpoint to update the status
      const response = await axios.put(`http://localhost:8070/api/bookings/update-status/${id}`, { status });
      alert('✅ Status updated!');
      fetchAppointments(); // Refresh the list of appointments after update
    } catch (error) {
  ...
  const deleteAppointment = async (id) => {
    try {
      ...
      await axios.delete(`http://localhost:8070/api/bookings/${id}`);
      setAppointments(appointments.filter((appt) => appt._id !== id)); // Remove from state
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gradient-to-r from-blue-600 to-indigo-700">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Service ID</th>
                ...
                <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider" style={{ width: '120px' }}>Actions</th> {/* Ensure this column has a proper width */}
              </tr>
            </thead>
                ...
                <tr key={appt._id} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        <div className="text-sm font-medium text-gray-900">{appt._id}</div> {/* Display the service ID here */}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {/* Dropdown always visible with current status */}
                    <div className="flex items-center">
                      <select
                        value={appt.status || 'Not Started'}
                        onChange={(e) => {
                          updateStatus(appt._id, e.target.value); // Update status when changed
                        }}
